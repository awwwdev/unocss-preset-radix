...
export function detectAndAddToColorsInUse<T>({
  ...
  return [
    ...
    (match, context: RuleContext) => {
      ...
      return AddToColorInUse({ token, property, hueOrAlias, shade, alpha, useP3Colors, prefix });
    },
  ];
};
...
export function AddToColorInUse({
  token,
  hueOrAlias,
  shade,
  alpha = '',
  ...
  token: Token;
  hueOrAlias: HueOrAlias;
  shade: Shade;
  alpha?: Alpha;
  useP3Colors: boolean;
}): string {
  console.log("🚀 ~ hueOrAlias:", hueOrAlias)
